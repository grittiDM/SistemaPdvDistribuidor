...
import br.pdvcongelados.model.services.ServicoVenda;
...
public class NovaVenda extends javax.swing.JFrame {
    ...
    private Venda venda;
    private boolean proximaVenda = true;
    ...
    public void finalizarVenda() {
        ...
        dtm.setRowCount(0);
        ServicoVenda.confirmSale(venda);
    ...
    private void btn_searchCodActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_searchCodActionPerformed
        produto = ProdutoDao.findById(Utils.tryParseToLong(txt_Code.getText()));
    ...
    private void btn_addToCartActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_addToCartActionPerformed
        ...
        if (quantity > 0 && produto != null) {
            try {
                item = new SaleItem(produto, quantidadeEstoque);
                venda.addItem(item);

                Utils.updateTable(venda.getItems(), table_ShoppingCart);
                ...
                clearProductFields();
                txt_SalesTotal.setText(String.format("R$ %.2f", venda.getValorTotal()));
    ...
    private void btn_searchCPFActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_searchCPFActionPerformed
        cliente = ClienteDao.findByCpf(txt_CPF.getText());
        if (cliente == null) {
    ...
    private void btnDeleteProductActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteProductActionPerformed
        ...
        if (selectedRow > -1) {
            ...
            int option = JOptionPane.showConfirmDialog(this, "Você tem certeza que quer remover o produto "
                    + "\'" + p.getDescricao() + "\'" + " da lista?", "Atenção!", JOptionPane.INFORMATION_MESSAGE);
                    
            if (option != 0) return;
            
            venda.getItems().removeIf(x -> x.getProduct().equals(p));
            Utils.updateTable(venda.getItems(), table_ShoppingCart);
            txt_SalesTotal.setText(String.format("R$ %.2f", venda.getTotal()));
